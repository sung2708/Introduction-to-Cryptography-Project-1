...
bool millerRabinTest(BigInt n, int k) {
  if (n == BigInt("2") || n == BigInt("5"))
    return true;
  if (n < BigInt("2") || n % BigInt("2") == BigInt("0"))
    return false;
  ...
  for (int i = 0; i < k; i++) {
    BigInt a = BigInt(std::to_string(rand() % (n.toInt() - 3) + 2));
...
int main(int argc, char *argv[]) {
  ...
  std::string hexValue;
  inputFile >> hexValue;
  BigInt decValue = BigInt(convertHexToDec(hexValue));

  if (millerRabinTest(decValue, 3)) {
    outputFile << "1";
  } else {
    outputFile << "0";
  }
